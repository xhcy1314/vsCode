...
function didChangeConfigurationListener() {
    ...
    const status = vscode_extensions_1.getConfig().inspect(constants_1.constants.vscode.iconThemeSetting).globalValue ===
        constants_1.constants.extensionName
...
function updatePreset(preset, toggledValue, global = true) {
    const removePreset = vscode_extensions_1.getConfig().inspect(`vsicons.presets.${preset}`).defaultValue ===
        toggledValue;
...
function showCustomizationMessage(message, items, callback, ...args) {
    customMsgShown = true;
    return vscode.window.showInformationMessage(message, ...items).then(btn => handleAction(btn, callback, ...args), 
        ...
        case i18nManager.getMessage(models.LangResourceKeys.autoReload):
            {
                vscode_extensions_1.getConfig()
                    .update(constants_1.constants.vsicons.projectDetectionAutoReloadSetting, true, true)
                    .then(() => handlePreset());
            }
            break;
